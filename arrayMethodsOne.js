...
// CODE HERE
const evenNumbers  = mixedNumbers.filter((element, index, wholeArray)=>{ return element % 2 === 0})
...
// CODE HERE
const postTaxPrices = prices.map( (element, index, wholeArray) => { return element * 1.07});
...
// CODE HERE
const totalPopulation = populations.reduce((acc, cur) => {return acc + cur})
...
// CODE HERE
const totals = orders.map((element, index, wholeArray)=>{return element.price * (1 + element.tax)})
console.log(totals)
...
// CODE HERE
bobsTotal = purchases.filter((element)=>{
  return element.owner ==="Bob"
}).reduce((acc, cur)=>{return acc + cur.price}, 0)
